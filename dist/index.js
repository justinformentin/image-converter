...
async function* walkImageFiles(folderPath, recursive) {
...
function getUntrackedImageFiles(folderPath) {
    try {
...
async function processImage(filePath, options) {
    ...
    const savedPercent = originalSize === 0 ? 0 : Number(((savedBytes / originalSize) * 100).toFixed(1));
    return {
        sourcePath: filePath,
        outputPath,
        originalSize,
...
export async function convertImages(inputOptions = {}) {
    ...
    log("");
    const imageFiles = options.untrackedFilesOnly
    ...
    const results = await processWithConcurrency(imageFiles, options.concurrency, async (imageFile) => {
        ...
        try {
            ...
            log(`✓ ${path.basename(result.outputPath)} ${formatBytes(result.originalSize)} → ${formatBytes(result.outputSize)} (${result.savedPercent}% saved)`);
            return result;
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
